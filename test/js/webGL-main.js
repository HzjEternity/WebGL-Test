...
class webGL{
    ...
    // 绘制
    webGl_Graphing () {
        const webGl = this.canvas.getContext('webgl') || this.canvas.getContext("experimental-webgl")

        // 创建、编译顶点着色器对象
        const vertexShader = this.createShader(webGl, 'VERTEX_SHADER', this.vertexShader)
        // 创建、编译片元着色器对象
        const fragmentShader = this.createShader(webGl, 'FRAGMENT_SHADER', this.fragmentShader)
        // 创建着色器程序
